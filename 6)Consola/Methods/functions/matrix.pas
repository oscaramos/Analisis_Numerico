...
type
  arr_of_arr_double = array of array of double;

  TMatrix = class
    private
      _matrix: arr_of_arr_double;
      _rows, _columns: Integer;
    public
      constructor create(rows: Integer; columns: Integer);
      constructor create(rows: Integer; columns: Integer; rawMatrix: arr_of_arr_double);
      ...
      function det(): Double;
      function triang(m: TMatrix): Double;
  ...
  operator-(a,b: TMatrix): TMatrix;
  operator*(a,b: TMatrix): TMatrix;
  operator*(a:TMatrix; b:Double): TMatrix;
  operator*(b:Double; a:TMatrix): TMatrix;
  operator/(a,b: TMatrix): TMatrix;
...
implementation
  ...
  constructor TMatrix.create(rows: Integer; columns: Integer; rawMatrix: arr_of_arr_double);
  begin
      _matrix := rawMatrix;
      _rows := rows;
      _columns := columns;
  ...
  begin
    ...
    d := self.det();
    if(d = 0) then begin
       raise Exception.create('TMatrix.inverse: La matriz no tiene inversa');
       exit();
    end;
...
function TMatrix.triang(m: TMatrix): Double;
  var
  i,j:Integer;
  upper,lower:TMatrix;
begin
  //Evalua que sea una matriz cuadrada
  if(m._rows <> m._columns) then
      raise Exception.create('TMatrix.triang: La matriz por parametro no es cuadrada');
  //Caso de que sea 2x2
  if(m._rows = 2) then
      Result:=m._matrix[0][0]*m._matrix[1][1]-m._matrix[0][1]*m._matrix[1][0];
  //Triangularizacion
  //Set de tamanios Upper y Lower
  Upper := TMatrix.create(m._rows, m._columns);
  Lower := TMatrix.create(m._rows, m._columns);
  //Lower y Upper
  for i:=0 to m._rows-1 do
    for j:=0 to m._rows-1 do
    begin
        if(i<=j) then
        begin
          upper._matrix[i,j]:=m._matrix[i,j];
          lower._matrix[i,j]:=0;
        end
        else
        begin
          upper._matrix[i,j]:=0;
          lower._matrix[i,j]:=m._matrix[i,j];
        end;
        if(i=j) then
        lower._matrix[i,j]:=1;
    end;
  ...
  for i:=0 to m._rows-1 do
      ...
      Result:=Result*upper._matrix[i,i];
end;
