...
uses
  ...
  TASources, TATools  , TACustomSeries, integrales;
...
type
  ...
  TfrmGraficadora = class(TForm)
    ...
    rdbIntegral: TRadioButton;
    rdbArea: TRadioButton;
    RadioGroup1: TRadioGroup;
    rdbTrapecio: TRadioButton;
    rdbSimpson: TRadioButton;
    ...
    procedure ediIntervaloChange(Sender: TObject);
...
var calculateIntegral : Boolean;
begin
 intg_trapecio := TMethodIntegral.create;
 with intg_trapecio do begin
   a := StrToFloat(AreaA.text);
   b := StrToFloat(AreaB.text);
   n := StrToInt(ediN.text);
   setExpr(cboFuncion.Text);
   if rdbTrapecio.Checked = True then
      MethodType := Trapecio
   else
      MethodType := Simpson;
   calculateIntegral := rdbIntegral.Checked = True;
   if calculateIntegral then
      FindArea := False
   else
      FindArea := True;
 end;
 Area.Active:= true;
 if calculateIntegral then
    lblShowSomething.Caption := 'Integral = '
 else
    lblShowSomething.Caption := 'Area = ';
 lblRpta.Caption := FloatToStr(intg_trapecio.evaluate());
...
procedure TfrmGraficadora.ediIntervaloChange(Sender: TObject);
begin
