...
type
  ...
  private
    ...
    function heunMethod(): Double;
    function rungeKutta3Method(): Double;
    function rungeKutta4Method(): Double;
...
begin
  case methodType of
    ...
    Heun:          Result := heunMethod();
    RungeKutta3:   Result := rungeKutta3Method();
    RungeKutta4:   Result := rungeKutta4Method();
...
function TMethodsDif.heunMethod(): Double;
...
function TMethodsDif.rungeKutta3Method(): Double;
const
  num_cols = 5; YnCol = 0;
  K1Col = 1; K2Col = 2; K3Col = 3;
  mCol = 4;
var
  Xn, Yn: Double;
  m, K1, K2, K3: Double;
  ...
  procedure addRow(Yn, K1, K2, K3, m: Double);
  begin
    table[YnCol].Add(FloatToStr(Yn));
...
begin
  ...
  for i:=1 to n do begin
     ...
     K3 := f(Xn + h, Yn - K1*h + 2*K2*h);
     m  := (h/6) * (K1 + 4*K2 + K3);
     Yn := op(Yn, h*m);
     Xn := op(Xn, h);
     addRow(Yn, K1, K2, K3, m);
  end;
  Result := Yn;
end;

function TMethodsDif.rungeKutta4Method(): Double;
const
  num_cols = 6; YnCol = 0;
...
begin
  ...
  for i:=1 to n do begin
     ...
     Yn := op(Yn, ((35*K1)/384 +  (500*K3)/1113  + (125*K4)/192 - (2187*K5)/6784  + (11*K6)/84));
     Xn := op(Xn, h);
